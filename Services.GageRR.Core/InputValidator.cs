...
namespace Services.GageRR.Core
{
    ...
    {
        public void Validate(RangeInput input)
        {
            if (input == null)
            ...
            if (input.Records.Count != input.AppraiserCount * input.PartCount)
                throw new GageException("측정값의 개수가 잘못되었습니다.");
            if (input.Records.GroupBy(x => x.Appraiser).Any(x => x.Count() != input.PartCount))
                throw new GageException("평가자별 측정값의 개수가 잘못되었습니다.");
            if (input.Records.GroupBy(x => x.Part).Any(x => x.Count() != input.AppraiserCount))
        ...
        public void Validate(AnovaInput input)
        {
            if (input == null)
                throw new GageException("입력값이 없습니다.");
